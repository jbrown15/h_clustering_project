...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for pSelf in self.points:
            for pOther in other.points:
                if initialMin == False:
                    minDist = pSelf.distance(pOther)
                    initialMin = True
                else:
                    if pSelf.distance(pOther) < minDist:
                        minDist = pSelf.distance(pOther)
        return minDist
    ...
    def maxLinkageDist(self, other):
        ...
        for pSelf in self.points:
            for pOther in other.points:
                if initialMax == False:
                    maxDist = pSelf.distance(pOther)
                    initialMax = True
                else:
                    if pSelf.distance(pOther) > maxDist:
                        maxDist = pSelf.distance(pOther)
        return maxDist
    ...
    def averageLinkageDist(self, other):
        ...
        for pSelf in self.points:
            for pOther in other.points:
                pairDist.append(pSelf.distance(pOther))
                
        averageDist = sum(pairDist)/len(pairDist)
        return averageDist
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        points =  list(set(c1.points) | set(c2.points))
...
PATH_TO_FILE = 'C:\Program Files\Enthought\CompThink\week6\cityTemps.txt'
#PATH_TO_FILE2 = 'C:\Program Files\Enthought\CompThink\week6\test.txt'
def test():
    #points = buildCityPoints(PATH_TO_FILE, False)
    #hCluster(points, Cluster.singleLinkageDist, 10, False)
    points = buildCityPoints(PATH_TO_FILE, True)
